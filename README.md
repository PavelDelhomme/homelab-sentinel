...
|-------------------|-------------|
| **Makefile** | Déploiement depuis le PC : **`make install`** (init sur la Pi), **`make update`** ou **`make push`** (mise à jour + redémarrage backend Docker). Voir [docs/workflow_pc_vers_pi.md](docs/workflow_pc_vers_pi.md). |
...
| `docs/cursor_remote_ssh_pi.md` | **Workflow PC → Pi** : Cursor Remote SSH **ne marche pas** sur Pi 2 (armv7l). Coder sur le PC dans Cursor, **make push** pour déployer ; config `~/.ssh/config` (Host pi-homelab) pour **ssh pi-homelab** (terminal, nano). Alias **homelab-push**. |
...
- **Tout depuis ton PC** : à la racine du dépôt, **`make bootstrap`** une fois (mot de passe pavel à la demande), redémarre la Pi, puis **`make install`**. Ensuite **`make push`** ou **`make update`** pour mettre à jour la Pi et redémarrer le backend. Alias **`homelab-push`** possible (voir [docs/cursor_remote_ssh_pi.md](docs/cursor_remote_ssh_pi.md)). Détail : [docs/workflow_pc_vers_pi.md](docs/workflow_pc_vers_pi.md).
...
- **Backend** : depuis la racine du repo, **`make push`** (déploie sur la Pi) ou en local `cd backend && docker compose up -d`.
